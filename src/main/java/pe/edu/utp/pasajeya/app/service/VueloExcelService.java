...
public class VueloExcelService {
    ...
    public byte[] generarExcel(List<VueloDTO> vuelos) throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            log.info("Excel generado — {} filas", vuelos.size());
    ...
    private void llenarDatos(Workbook workbook, Sheet sheet, List<VueloDTO> vuelos) {
        ...
        for (VueloDTO v : vuelos) {
            ...
            fila.createCell(9).setCellValue(v.equipajeBodegaKg() != null ? v.equipajeBodegaKg() : 0);
            fila.createCell(10).setCellValue(v.equipajeManoKg() != null ? v.equipajeManoKg() : 0);
            fila.createCell(11).setCellValue(Boolean.TRUE.equals(v.permiteReembolso()) ? "Sí" : "No");
            fila.createCell(12).setCellValue(Boolean.TRUE.equals(v.asientoSeleccionable()) ? "Sí" : "No");
            fila.createCell(13).setCellValue(v.semaforo().toUpperCase());
